...
@section('content')
  <div class="container-fluid p-4">
    ...
    <p>Modifica le informazioni del tuo ristorante</p>
    ...
    <form name="restaurantForm" onsubmit="return validateRestaurantForm()"
      ...
      <div class="row mb-2">
        <div class="col-6">
          ...
          <input type="text" class="form-control @error('address') is-invalid @enderror" id="address" name="address"
            value="{{ old('address', $restaurant->address) }}" placeholder="es.Via Italia n* 6" required>
          @error('address')
            <div class="alert alert-danger">{{ $message }}</div>
          @enderror
        </div>
        <div class="col-3">
          ...
          <input type="tel" class="form-control @error('phone_number') is-invalid @enderror" id="phone_number"
            name="phone_number" value="{{ old('phone_number', $restaurant->phone_number) }}" placeholder="es.3334567810">
          @error('phone_number')
            <div class="alert alert-danger">{{ $message }}</div>
          @enderror
        </div>
        ...
        @else
          ...
          <div class="d-flex gap-3">
            @foreach ($typeList as $type)
              <div class="form-check text-center">
                <input name="typeList[]" class="form-check-input" type="checkbox" value="{{ $type->id }}"
                  id="type-{{ $type->id }}" {{ $restaurant->types()->find($type->id) ? 'checked' : '' }} />
                <label class="form-check-label" for="type-{{ $type->id }}">{{ $type->name }}</label>
              </div>
            @endforeach
          </div>
      ...
      <div class="mb-2">
        ...
        <input type="text" class="form-control @error('vat') is-invalid @enderror" id="vat" name="vat"
          value="{{ old('vat', $restaurant->vat) }}" placeholder="es.11111111111" required>
        @error('vat')
          <div class="alert alert-danger">{{ $message }}</div>
        @enderror
      </div>
