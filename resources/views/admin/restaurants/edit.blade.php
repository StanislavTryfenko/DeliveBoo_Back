...
@section('content')
    <section class="p-2">
        ...
        <form name="restaurantForm" onsubmit="return validateRestaurantForm()" action="{{ route('admin.restaurants.update', $restaurant) }}" method="POST"
    ...
    <script>
        function validateRestaurantForm() {        
           ...
           /*  if (!phone_number.match(/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/im)) {
            ...
            if (phone_number.length != 10 || !phone_number.match(/^[0-9]+$/)) {
                alert("Please enter a valid phone number");
                return false;
            }
            ...
            if (vat.length != 11 || !vat.match(/^[0-9]+$/)) {
